...
// 查询所有用户信息
func SelectAllrecord(userId string) (*[]model.RecordHTML, error) {
	var recordsList = make([]model.RecordHTML, 0, 20) //传一个切片
	var record model.RecordHTML
	sqlStr := `SELECT r.id,r.user_id,r.book_id,r.tradingTime,u.username,u.reputation,b.title
	FROM  tab_record as r
	LEFT JOIN tab_books b
	ON r.book_id = b.id
	LEFT JOIN tab_users u
	ON r.user_id = u.id where r.user_id=?;`
	...
	// 4,循环取值
	for rows.Next() {
		err = rows.Scan(&record.ID, &record.User_id, &record.Book_id, &record.TradingTime, &record.Username, &record.Reputation, &record.Title)
		if err != nil {
...
// 添加一条
func AddRecordDB(newRecord *model.Record) (err error) {
	// 获取现在时间日期
...
//
func SelectLikerecord(value *string) (*[]model.RecordHTML, error) {

	var recordList = make([]model.RecordHTML, 0, 20)
	var record model.RecordHTML //传一个切片
